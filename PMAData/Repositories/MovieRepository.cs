        ...
        private void DoCreateOrUpdate(Movie movie, int retry = 0)
        {
            try
            {
                using (var conn = databaseConfig.NewConnection())
                {
                    ...
                    if (dbMovie != null)
                    {
                        sql = "UPDATE Movie SET Title = @Title, Year = @Year, LastPlayed = @LastPlayed, Added = @Added, FileSize = @FileSize, IsCurrent = @IsCurrent, IsArchived = @IsArchived, WHERE ID = @ID;";
                    }
                    else
                    {
                        sql = "INSERT INTO Movie (ID, Title, Year, LastPlayed, Added, FileSize, IsCurrent, IsArchived) VALUES (@ID, @Title, @Year, @LastPlayed, @Added, @FileSize, @IsCurrent, @IsArchived);";
                    }
